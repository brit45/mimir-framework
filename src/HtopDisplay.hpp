...
public:
    ...
    std::string progressBar(float percent, int bar_width = 40, bool show_percent = true) {
        ...
        for (int i = 0; i < bar_width; ++i) {
            if (i < filled) {
                if (percent > 80.0f) ss << colorText("█", 32); // Vert
                else if (percent > 50.0f) ss << colorText("█", 33); // Jaune
                else ss << "█";
            }
            else if (i == filled) ss << "▓";
            else ss << "░";
    ...
    std::string memoryBar(size_t used_mb, size_t total_mb, int bar_width = 30) {
        ...
        for (int i = 0; i < bar_width; ++i) {
            if (i < filled) {
                if (percent > 100.0f) ss << colorText("█", 91); // Rouge clignotant
                else if (percent > 80.0f) ss << colorText("█", 31); // Rouge
                else if (percent > 60.0f) ss << colorText("█", 33); // Jaune
                else ss << colorText("█", 32); // Vert
            } else {
                ss << "░";
            }
        }
    ...
    std::string metricBar(const std::string& label, float value, float scale = 1.0f, int bar_width = 25) {
        ...
        for (int i = 0; i < bar_width; ++i) {
            if (i < filled) {
                if (percent > 75.0f) ss << colorText("█", 31); // Rouge = élevé
                else if (percent > 50.0f) ss << colorText("█", 33); // Jaune
                else if (percent > 25.0f) ss << colorText("█", 32); // Vert
                else ss << "▓";
            } else {
                ss << "░";
            }
    ...
    void render() {
        ...
        // Header
        std::string header = "= FLUX Diffusion Trainer =";
        int padding = (width - header.length()) / 2;
        moveCursor(row++, padding);
        std::cout << colorText(header, 96) << std::endl;
        ...
        std::cout << colorText("Uptime:", 36) << " " << formatTime(elapsed) 
                  << "  |  " << colorText("ETA:", 36) << " " << formatTime(stats.eta_seconds) << std::endl;
        
        moveCursor(row++, 1);
        std::cout << std::string(width - 2, '_') << std::endl;
        
        // Epoch Progress
        moveCursor(row++, 2);
        std::cout << colorText("EPOCH", 33) << " " << stats.current_epoch << "/" << stats.total_epochs << std::endl;
        float epoch_percent = (float)stats.current_epoch / stats.total_epochs * 100.0f;
        moveCursor(row++, 2);
        std::cout << progressBar(epoch_percent, width - 15) << std::endl;
        
        // Batch Progress
        moveCursor(row++, 2);
        std::cout << colorText("BATCH", 33) << " " << stats.current_batch << "/" << stats.total_batches << std::endl;
        float batch_percent = (float)stats.current_batch / stats.total_batches * 100.0f;
        moveCursor(row++, 2);
        std::cout << progressBar(batch_percent, width - 15) << std::endl;
        ...
        // System Resources
        moveCursor(row++, 2);
        std::cout << colorText("● SYSTEM RESOURCES", 95) << std::endl;
        
        moveCursor(row++, 4);
        std::cout << "Memory: " << memoryBar(stats.memory_used_mb, 10240, 30) << std::endl;
        
        moveCursor(row++, 4);
        std::cout << "Freed this batch: " << colorText(std::to_string(stats.memory_freed_mb) + " MB", 92) << std::endl;
        
        moveCursor(row++, 4);
        std::cout << "Parameters: " << colorText(std::to_string(stats.total_params), 96) 
                  << " (~" << std::fixed << std::setprecision(2) 
                  << (stats.total_params * 2.0f / 1024.0f / 1024.0f / 1024.0f) << " GB)" << std::endl;
        ...
        // Training Info
        moveCursor(row++, 2);
        std::cout << colorText("● TRAINING INFO", 95) << std::endl;
        
        moveCursor(row++, 4);
        std::cout << "Learning Rate: " << colorText(std::to_string(stats.learning_rate), 93) << std::endl;
        
        moveCursor(row++, 4);
        std::cout << "Timestep (t): " << colorText(std::to_string(stats.timestep).substr(0, 6), 96) << std::endl;
        
        moveCursor(row++, 4);
        std::cout << "Batch Time: " << colorText(std::to_string(stats.batch_time_ms) + " ms", 92) << std::endl;
        
        moveCursor(row++, 4);
        std::cout << "Speed: " << colorText(std::to_string(stats.batches_per_sec).substr(0, 5) + " batches/s", 92) << std::endl;
        ...
        // Footer
        moveCursor(row++, 2);
        std::cout << colorText("Press Ctrl+C to stop", 90) << std::endl;
