...
void VGG19Model::buildInto(Model& model, const Config& cfg) {
    ...
    // VGG19 conv counts: [2,2,4,4,4]
    auto block = [&](int bi, int convs, int out_c) {
        const std::string p = "vgg19/b" + std::to_string(bi);
    ...
    model.push("vgg19/gap", "GlobalAvgPool2d", 0);
    if (auto* L = model.getLayerByName("vgg19/gap")) {
        L->inputs = {x};
        L->output = "vgg19/gap_y";
        L->in_channels = cur_c;
        L->input_height = cur_h;
        L->input_width = cur_w;
    }

    model.push("vgg19/fc1", "Linear", static_cast<size_t>(cur_c) * static_cast<size_t>(fc_hidden) + static_cast<size_t>(fc_hidden));
    if (auto* L = model.getLayerByName("vgg19/fc1")) {
        L->inputs = {"vgg19/gap_y"};
        L->output = "vgg19/fc1_y";
        L->in_features = cur_c;
        L->out_features = fc_hidden;
        L->use_bias = true;
    }
    model.push("vgg19/fc1_act", "GELU", 0);
    if (auto* L = model.getLayerByName("vgg19/fc1_act")) {
        L->inputs = {"vgg19/fc1_y"};
        L->output = "vgg19/fc1_h";  // tenseur renommé: évite collision layer-name == tensor-name
    }
