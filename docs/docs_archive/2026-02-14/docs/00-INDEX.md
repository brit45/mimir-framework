...
- 🧩 **[Lua Scripting](01-Getting-Started/07-Lua-Scripting.md)** - Écrire et exécuter des scripts
...
- ⚙️ **[Runtime en pratique](02-User-Guide/11-Runtime-In-Practice.md)** - Démarrage, args Lua, mémoire, perf
...
- 🌐 **[Serveur REST/WebSocket](03-API-Reference/01-API-Quick-Reference.md#serveur-rest--websocket-lua)** - Pilotage JSON + auth/TLS
...
**Modules disponibles (API Lua):**

- `Mimir.Model` / alias global `model` - Gestion du modèle (create/allocate/init/forward/backward/train)
- `Mimir.Architectures` / alias global `architectures` - Helpers de registre (`available`, `default_config`)
- `Mimir.Layers` - Ajout et inspection de couches
- `Mimir.Tokenizer` / alias global `tokenizer` - Tokenization
- `Mimir.Dataset` / alias global `dataset` - Chargement/accès dataset
- `Mimir.Memory` - Gestion mémoire avancée
- `Mimir.Guard` et `Mimir.Checkpoint` - Legacy (compat)
- `Mimir.MemoryGuard` / alias global `MemoryGuard` - Limites RAM et monitoring
- `Mimir.Allocator` / alias global `Allocator` - Configuration allocateur
- `Mimir.Htop` / alias global `htop` - Monitoring terminal
- `Mimir.Viz` / alias global `viz` - Visualisation (SFML)
- `Mimir.Serialization` - Sérialisation moderne (save/load/detect_format/save_enhanced_debug)
- Globales : `log()`, `read_json()`, `write_json()`

**Note:** la source de vérité de l'API exportée est `src/LuaScripting.cpp`.
...
- 🌊 **[Modèle Flux](04-Architecture-Internals/10-Flux-Model.md)** - Historique (obsolète v2.3)
- 🌊 **[Implémentation Flux](04-Architecture-Internals/10-Flux-Implementation.md)** - Historique (obsolète v2.3)
...
✅ **11 architectures intégrées** - `basic_mlp`, `t2i_autoencoder`, `transformer`, `vit`, `vae`, `resnet`, `unet`, `mobilenet`, `vgg16`, `vgg19`, `diffusion`  
...
local ok_alloc, num_params = Mimir.Model.allocate_params()
if not ok_alloc then error("allocate_params failed") end
Mimir.Model.init_weights("xavier", 1234)
print("✅ Modèle créé avec", num_params, "paramètres!")
EOF

# 3. Exécuter
./bin/mimir --lua hello_mimir.lua
```
...
- 📧 **Email** : [support@mimir-framework.org](mailto:support@mimir-framework.org)
