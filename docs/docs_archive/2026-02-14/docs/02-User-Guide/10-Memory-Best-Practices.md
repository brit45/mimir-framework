...
cfg.vocab_size = 50000       -- 50k tokens OK
cfg.d_model = 768            -- GPT-2 small size
cfg.num_layers = 12          -- 12 couches OK
cfg.num_heads = 12
...
-- ResNet-50
local resnet_cfg, err = Mimir.Architectures.default_config("resnet")
if not resnet_cfg then error(err) end
resnet_cfg.num_classes = 1000
assert(Mimir.Model.create("resnet", resnet_cfg))
Mimir.Model.allocate_params()  -- ~25M params = ~100 MB OK

-- UNet pour segmentation
local unet_cfg, err = Mimir.Architectures.default_config("unet")
if not unet_cfg then error(err) end
unet_cfg.in_channels = 3
unet_cfg.out_channels = 1
assert(Mimir.Model.create("unet", unet_cfg))
Mimir.Model.allocate_params()  -- ~30M params OK
```
...
local diff_cfg, err = Mimir.Architectures.default_config("diffusion")
if not diff_cfg then error(err) end
diff_cfg.image_size = 256

assert(Mimir.Model.create("diffusion", diff_cfg))
...
local cfg1 = assert(Mimir.Architectures.default_config("transformer"))
cfg1.d_model = 1024
cfg1.num_layers = 12
assert(Mimir.Model.create("transformer", cfg1))
Mimir.Model.allocate_params()

Mimir.Model.create("model2")  -- Le premier est toujours en mémoire!
local cfg2 = assert(Mimir.Architectures.default_config("transformer"))
cfg2.d_model = 1024
cfg2.num_layers = 12
assert(Mimir.Model.create("transformer", cfg2))
