...
#### Ajouts (Nouvelle API v2.2)
```lua
-- API Serialization v2.2
...
```cpp
static int lua_saveCheckpoint(lua_State* L);    // Serialization API v2.2
static int lua_loadCheckpoint(lua_State* L);    // Serialization API v2.2
static int lua_detectFormat(lua_State* L);      // Serialization API v2.2
...
#### Exemple Complet
**Fichier:** `scripts/examples/example_serialization_v2.2.lua`
...
> ⚠️ Note (code actuel v2.3) : ce document est conservé à titre historique. Les modules `Mimir.Flux` / `Mimir.FluxModel` ne sont **pas exportés** par l'API Lua du binaire actuel.

### Modules Exposés (réalité du binaire actuel)

1. **Mimir.Model** (alias global: `model`) - Gestion du modèle
2. **Mimir.Architectures** (alias global: `architectures`) - Registre (liste + config par défaut)
3. **Mimir.Layers** (alias global: `layers`) - Opérations layers
4. **Mimir.Tokenizer** (alias global: `tokenizer`) - Tokenization
5. **Mimir.Dataset** (alias global: `dataset`) - Données
6. **Mimir.Memory** (alias global: `Memory`) - RAM manager
7. **Mimir.Guard** (alias global: `guard`) - MemoryGuard legacy
8. **Mimir.MemoryGuard** (alias global: `MemoryGuard`) - Limites + stats
9. **Mimir.Allocator** (alias global: `Allocator`) - Allocateur dynamique
10. **Mimir.Htop** (alias global: `htop`) - Monitoring
11. **Mimir.Viz** (alias global: `viz`) - Visualisation
12. **Mimir.Serialization** - Sérialisation (save/load/detect_format)
...
2. **scripts/examples/example_serialization_v2.2.lua** - Exemple complet
3. **scripts/templates/template_new_model.lua** - Template mis à jour
4. **scripts/migrate_to_serialization_v2.2.sh** - Script migration
...
### Immédiat
1. ✅ Tester `example_serialization_v2.2.lua`
...
- **example_serialization_v2.2.lua:** +250 lignes (exemple)
- **migrate_to_serialization_v2.2.sh:** +80 lignes (migration)

**Total:** ~500 lignes

### Documentation
- **Nouveau:** example_serialization_v2.2.lua (250 lignes)
