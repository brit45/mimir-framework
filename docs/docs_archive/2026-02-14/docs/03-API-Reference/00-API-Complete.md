...
### `Mimir.Model.set_hardware(enable)`

Active/désactive l'accélération matérielle côté modèle.

> Source de vérité: `src/LuaScripting.cpp` (binding `lua_setHardwareAccel`).
> Cette fonction **ne sélectionne pas** un backend nommé (CPU/OpenCL/Vulkan) : elle active/désactive un mode d'accélération.

**Paramètres:**
- `enable` (boolean) :
  - `true` : activer l'accélération matérielle (si disponible)
  - `false` : forcer le chemin CPU
...
2. **Accélération** : Utilisez `Mimir.Model.set_hardware(false)` pour forcer CPU, ou `Mimir.Model.set_hardware(true)` pour activer l'accélération si disponible
