...
> Note: une ancienne version de ce document mentionnait `src/Models/FluxModel.cpp`.
> Le code actuel ne contient pas de `FluxModel` dans `src/` et cette modification n'est pas applicable.
...
4. `docs/BRANCH_OPERATIONS.md` - Documentation complète (nouveau)
5. `scripts/demo_branches.lua` - Script de démonstration (nouveau)
