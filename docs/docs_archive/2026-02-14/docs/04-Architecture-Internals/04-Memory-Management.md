...
### 1. Demo Simple
**Fichier** : `scripts/demos/demo_10gb_limit.lua`

```bash
./bin/mimir --lua scripts/demos/demo_10gb_limit.lua
```
...
### 2. Test Complet
**Fichier** : `scripts/tests/test_flux_complete.lua`

Ce script est un artefact historique : il détecte l'absence de `Mimir.FluxModel` dans l'API Lua v2.3 et fait un **skip propre**.
...
| `scripts/tests/test_flux_complete.lua` | Test legacy (skip si FluxModel absent) | - |
| `scripts/demos/demo_10gb_limit.lua` | Démo limite mémoire | - |
...
-- Votre code (exemple: création d'un modèle via le registre)
local name = "transformer"
