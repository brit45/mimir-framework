...
### 🎯 Objectif
Document historique (v2.1.0) sur la mise à jour du stub `mimir-api.lua`.

> ⚠️ En v2.3, `Mimir.Flux` / `Mimir.FluxModel` ne sont plus exportés par l’API Lua.
...
> ⚠️ Cette section concernait Flux/FluxModel dans une version antérieure.
> En v2.3, `Mimir.Flux` / `Mimir.FluxModel` ne sont **pas exportés** par l’API Lua (voir `src/LuaScripting.cpp`).
> Les tests associés existent toujours mais font un *skip* si ces modules sont absents.

Référence à jour : `docs/03-API-Reference/00-API-Complete.md`.
...
---@type FluxModelAPI
-- (historique) FluxModel n'est pas exporté en v2.3
-- FluxModel = FluxModel
```
...
1. **Autocomplétion complète** pour `Mimir.MemoryGuard.*` (v2.3)
