...
Mímir propose **9 architectures prédéfinies** accessibles via l'API `architectures` :
...
local unet = Mimir.Architectures.unet({
    ...
    depth = 4             -- Profondeur encoder/decoder
})
```
...
|-----------|------|--------|-------------|
| `in_channels` | int | 3 | Canaux d'entrée (1=grayscale, 3=RGB) |
| `out_channels` | int | 1 | Canaux de sortie (classes de segmentation) |
| `base_channels` | int | 64 | Nombre de filtres de base (doublé à chaque niveau) |
| `depth` | int | 4 | Profondeur de l'encoder/decoder |
| `use_batchnorm` | bool | true | Utiliser Batch Normalization |
