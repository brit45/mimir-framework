...
local function die(msg)
  local text = "[hf_clip_text_encoder_1_infer] ❌ " .. tostring(msg)
...
local cfg, err_cfg = Mimir.Architectures.default_config("hf_clip_text_encoder_1")
if type(cfg) ~= "table" then die("default_config(hf_clip_text_encoder_1) a échoué: " .. tostring(err_cfg)) end
cfg.seq_len = seq_len
...
ok_or_die(Mimir.Model.create("hf_clip_text_encoder_1", cfg), nil, "Model.create(hf_clip_text_encoder_1)")
apply_dtype(cfg)
...
logf("[hf_clip_text_encoder_1_infer] prompt=%s", tostring(prompt))
logf("[hf_clip_text_encoder_1_infer] ids=%d output=%d", #ids, #output)
logf("[hf_clip_text_encoder_1_infer] sequence_dim=%d", sequence_dim)
logf("[hf_clip_text_encoder_1_infer] seq_head=[%s]", dump_head(output, 8))